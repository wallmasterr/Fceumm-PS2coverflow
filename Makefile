...
# 5 random alphanumeric chars, new each time this header is regenerated (main/browser depend on it).
$(PS2_DIR)/build_stamp.h: FORCE
	@{ echo "/* Generated by Makefile; new ID each rebuild */"; \
	   echo "#ifndef LOWTEK_BUILD_STAMP_H"; \
	   echo "#define LOWTEK_BUILD_STAMP_H"; \
	   _ltek_id=$$(LC_ALL=C tr -dc 'A-Za-z0-9' </dev/urandom | head -c5); \
	   echo "#define LOWTEK_BUILD_ID \"$$_ltek_id\""; \
	   echo "#endif"; \
	} > $@

FORCE:

$(PS2_DIR)/main.o $(PS2_DIR)/browser.o: $(PS2_DIR)/build_stamp.h
...
clean:
	...
	$(RM) $(EE_BIN) $(EE_PACKED_BIN) $(EE_OBJS) $(PS2_DIR)/irx/*.c $(PS2_DIR)/build_stamp.h

#include Makefile.eeglobal
include $(PS2SDK)/samples/Makefile.pref
include $(PS2SDK)/samples/Makefile.eeglobal

# ps2-packer: SDK bin, ps2dev bin (sibling of ps2sdk), optional $$PS2DEV/bin, then PATH. Empty => skip pack.
PS2_PACKER := $(firstword $(wildcard $(PS2SDK)/bin/ps2-packer $(dir $(PS2SDK))bin/ps2-packer $(if $(strip $(PS2DEV)),$(PS2DEV)/bin/ps2-packer)))
ifeq ($(strip $(PS2_PACKER)),)
PS2_PACKER := $(shell command -v ps2-packer 2>/dev/null || true)
endif
...
elf_post:
	$(EE_STRIP) $(EE_BIN)
ifneq ($(NOT_PACKED),1)
ifneq ($(strip $(PS2_PACKER)),)
	$(PS2_PACKER) $(EE_BIN) $(EE_PACKED_BIN)
else
	$(RM) $(EE_PACKED_BIN)
	@echo "*** ps2-packer not found: built $(EE_BIN) only (no $(EE_PACKED_BIN)). Install tool or add to PATH; or make NOT_PACKED=1. https://github.com/ps2dev/ps2-packer ***"
endif
endif
